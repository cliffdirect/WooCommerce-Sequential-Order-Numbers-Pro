 ...
 * Description: Provides custom incrementing order numbers for WooCommerce orders.<a href="http://www.96down.com/">Woocommerce Extensions Download</a>.
 * Version: 1.5
 * Author: SkyVerge
 * Author URI: http://www.skyverge.com
 * Tested up to: 3.5
 *
 * Text Domain: wc-seq-order-numbers
...
/**
 * <h2>WooCommerce Sequential Order Numbers Main Plugin Class</h2>
 *
...
class WC_Seq_Order_Number_Pro {
	...
	/** Plugin text domain */
	const TEXT_DOMAIN = 'wc-seq-order-numbers';
	 ...
	 *        a post object when the order is created in the admin.
	 */
	...
	private function generate_sequential_order_number( $post_id, $order_number_meta_name, $order_number_start, $order_number_prefix = '', $order_number_suffix = '', $order_number_length = 1 ) {
		...
		for ( $i = 0; $i < 3 && ! $success; $i++ ) {
			...
			if ( $success ) {
				...
				$order_number = get_post_meta( $post_id, $order_number_meta_name, true );
	...
	public function get_order_number( $order_number, $order ) {
		...
		// maintain the hash?
		$maybe_hash = $this->get_has_hash_before_order_number() ? _x( '#', 'hash before order number', 'woocommerce' ) : '';

		if ( isset( $order->order_custom_fields['_order_number_formatted'] ) ) {
			return $maybe_hash . $order->order_custom_fields['_order_number_formatted'][0];
		}
	...
	public function validate_order_number_start_setting( $newvalue, $oldvalue ) {
		...
		if ( ! is_null( $order_number ) && (int) $order_number >= $newvalue ) {
			...
			$this->errors = sprintf( __( 'There is an existing order (%s) with a number greater than or equal to %s.  To set a new order number start please choose a higher number or permanently delete the relevant order(s).', self::TEXT_DOMAIN ), $order->get_order_number(), $newvalue );
			return $oldvalue;
		}
		...
		foreach ( $settings as $section ) {
			...
			if ( isset( $section['id'] ) && 'general_options' == $section['id'] &&
				...
				$updated_settings[] = array(
					...
					'desc_tip' => sprintf( __( 'The starting number for the incrementing portion of the order numbers, unless there is an existing order with a higher number.  Any newly placed orders will be numbered like: %s', self::TEXT_DOMAIN ), $this->format_order_number( get_option( 'woocommerce_order_number_start' ), $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length() ) ),
				...
				$updated_settings[] = array(
					'name'     => __( 'Order Number Length', self::TEXT_DOMAIN ),
					'desc_tip' => __( "This is the desired minimum length of the incrementing portion of the order number.  The numeric portion of the order number will be left-padded with 0's as necessary to achive the length configured here.", self::TEXT_DOMAIN ),
					'id'       => 'woocommerce_order_number_length',
					'type'     => 'text',
					'css'      => 'min-width:300px;',
					'default'  => 1, // WC >= 2.0
					'std'      => 1, // WC < 2.0
	...
	public function admin_settings_js() {
		...
		$('#woocommerce_order_number_start, #woocommerce_order_number_length, #woocommerce_order_number_prefix, #woocommerce_order_number_suffix').on('keyup change input',
			function() {
				$('#sample_order_number').text(formatOrderNumber($('#woocommerce_order_number_start').val(), $('#woocommerce_order_number_prefix').val(), $('#woocommerce_order_number_suffix').val(), $('#woocommerce_order_number_length').val()));
			}
		);
		function formatOrderNumber(orderNumber, orderNumberPrefix, orderNumberSuffix, orderNumberLength) {
			...
			orderNumberLength = (typeof orderNumberLength === "undefined") ?  1 : orderNumberLength;

			// order number padding?
			if (orderNumberLength && /^\d$/.test(orderNumberLength)) {
				orderNumber = leftPad(orderNumber, parseInt(orderNumberLength), '0');
			}
	...
	private function is_free_order( $order_id ) {
		...
		if ( $order->get_total() > 0 ) return false;

		// otherwise, lets make sure the items truly are free, and not just on sale, or discounted with a coupon
			...
			do {
				...
				if ( is_array( $order_ids ) ) {
					foreach( $order_ids as $order_id ) {
						if ( '' == ( $order_number = get_post_meta( $order_id, '_order_number', true ) &&
						     '' == get_post_meta( $order_id, '_order_number_formatted', true ) ) ) {
						...
						} elseif( '' == get_post_meta( $order_id, '_order_number_formatted', true ) ) {
